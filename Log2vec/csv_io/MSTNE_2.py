...
class MCTNE(torch.nn.Module):
    def __init__(self, file_path, emb_size=128, neg_size=5, hist_len=5, directed=False,
                 learning_rate=0.01, batch_size=1000, save_step=1, epoch_num=100):
    ...
    def forward(self, s_nodes, t_nodes, t_times, n_nodes, h_nodes, h_times, h_types, h_t_time, h_time_mask, h_t_masks):
    ...
    def loss_func(self, s_nodes, t_nodes, t_times, n_nodes, h_nodes, h_times, h_types, h_t_times, h_time_mask, h_t_masks):
        if torch.cuda.is_available():
    ...
    def update(self, s_nodes, t_nodes, t_times, n_nodes, h_nodes, h_times, h_types, h_t_times, h_time_mask, h_t_masks):
        if torch.cuda.is_available():
            self.opt.zero_grad()
            ...
            for i_batch, sample_batched in enumerate(loader):
                ...
                if torch.cuda.is_available():
                    self.update(sample_batched['source_node'].type(LType).cuda(),
                                ...
                                sample_batched['history_types'].type(FType).cuda(),
                                sample_batched['history_target_times'].type(FType).cuda(),
                                sample_batched['history_masks'].type(FType).cuda(),
                                sample_batched['history_t_masks'].type(FType).cuda())
                else:
